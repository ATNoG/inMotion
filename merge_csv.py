...
def merge_csv_files(input_dir: str, output_file: str, pattern: str = "*.csv") -> None:
    ...
    if "concurrent_noise_path" in merged_df.columns:
        ...
        if not concurrent_counts.empty:
            ...
            print(
                f"  None (no concurrent noise): {merged_df['concurrent_noise_path'].isna().sum()} samples"
            )
...
Examples:
  ...
  python merge_csv.py --input data/ --output fails.csv --pattern "*_fail*.csv"
    ...
    parser.add_argument(
        "--input", "-i", type=str, required=True, help="Input directory containing CSV files"
    )
    parser.add_argument("--output", "-o", type=str, required=True, help="Output merged CSV file")
    parser.add_argument(
        "--pattern", "-p", type=str, default="*.csv", help="File pattern to match (default: *.csv)"
    )
