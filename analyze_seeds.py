...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--seeds", type=str, nargs="+", default=SEEDS, help="Seeds used in experiments"
    )
    parser.add_argument(
        "--output-dir",
        type=str,
        default="seeds_results_combined",
        help="Output directory for combined results",
    )
    return parser.parse_args()


def load_seed_results(seeds: list[str]) -> dict[int, pd.DataFrame]:
    ...
    for seed in seeds:
        ...
        if results_path.exists():
            ...
            df["Seed"] = int(seed.replace("noise", "").replace("pure", ""))
            results[seed] = df
...
def compute_statistics(results: dict[int, pd.DataFrame]) -> pd.DataFrame:
    ...
    stats.columns = ["Classifier"] + [
        f"{m}_{s}" for m in available_metrics for s in ["Mean", "Std"]
    ]
...
def plot_seed_stability_ranking(
    ...
    plt.tight_layout()
    plt.savefig(
        output_dir / f"{metric.lower()}_ranking_stability.pdf", dpi=300, bbox_inches="tight"
    )
    plt.close()
