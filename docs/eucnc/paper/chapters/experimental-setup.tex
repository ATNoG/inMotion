...
\textbf{Matthews Correlation Coefficient (MCC):} Selected as the primary evaluation criterion following recommendations from Chicco and Jurman~\cite{chicco2020mcc}, who demonstrated that MCC produces reliable scores only when all four confusion matrix categories (true positives, true negatives, false positives, false negatives) achieve high values. Unlike accuracy and F1-score, MCC is robust to class imbalance and considers prediction performance across all classes proportionally. The coefficient ranges from $-1$ (complete disagreement) to $+1$ (perfect prediction), with $0$ indicating random chance. Chicco and Jurman further established that MCC should replace ROC-AUC as the standard metric for binary and multi-class evaluation~\cite{chicco2023mcc}, as a high MCC necessarily implies high values for sensitivity, specificity, precision, and negative predictive value—a guarantee not provided by other metrics.