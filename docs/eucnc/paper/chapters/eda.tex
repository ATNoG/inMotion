...
Ahead of classifier training, an exploratory data analysis was carried out to gauge the discriminative potential of temporal RSSI signatures and to characterize signal behaviour across different passenger movement classes.
...
The temporal evolution of RSSI values serves as the primary discriminative feature between movement classes. \autoref{fig:temporal_rssi} depicts the mean RSSI trajectory over the 10-second observation window for each class.
...
These contrasting temporal patterns offer a compelling rationale for leveraging RSSI sequences in movement classification.
...
\begin{figure}[!ht]
    \centering
    \includegraphics[width=0.9\linewidth]{images/tsne_visualization.pdf}
    \caption{t-SNE projection for visualization of each class structure}
    \label{fig:tsne}
\end{figure}

Although partial overlap is evident, mainly among samples collected under noisy conditions, the overall clustering suggests that temporal RSSI patterns retain sufficient class-dependent structure to support supervised learning approaches. This analysis serves as a qualitative inspection of feature separability rather than a quantitative performance evaluation.