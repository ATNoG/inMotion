...
\autoref{tab:top_classifiers} summarizes the performance of the top-10 classifiers on the combined dataset, ranked by mean \ac{MCC} across the experimental seeds.

\begin{table}[!ht]
    \centering
    \caption{Top-10 Classifiers on Combined Dataset (Ranked by \ac{MCC})}
...
The Gaussian Process classifier attained the highest mean \ac{MCC} (0.756) with an accuracy of 81.6\%. \acp{SVM} with \ac{RBF} and linear kernels followed closely, recording \ac{MCC} values of 0.755 and 0.750, respectively.
...
The \ac{GP} classifier employed a \ac{RBF} kernel, which is well-suited for capturing non-linear relationships in the \ac{RSSI} feature space. The model configuration used is presented in \autoref{tab:hyperparams}.
...
The \ac{RBF} kernel automatically learns the optimal length scale parameter during training, adapting to the intrinsic dimensionality of the \ac{RSSI} temporal sequences.
...
\begin{figure}[!ht]
    ...
    \caption{Mean feature importance across classifiers, highlighting the discriminative value of initial \ac{RSSI} measurements.}
