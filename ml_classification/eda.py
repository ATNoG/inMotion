...
class ExploratoryDataAnalysis:
    ...
    def plot_rssi_distributions(self, df: pd.DataFrame, save_path: Path | None = None) -> None:
        ...
        for i, cls in enumerate(df[self.config.target_column].unique()):
            ...
            axes[1, 0].violinplot(
                class_features, positions=np.arange(10) + i * 12,
        ...
        axes[1, 1].axvline(np.mean(all_rssi), color="red", linestyle="--", label=f"Mean: {np.mean(all_rssi):.1f}")
        axes[1, 1].legend()
    ...
    def plot_pca_visualization(
        ...
        for i, cls in enumerate(np.unique(y)):
            ...
            axes[0].scatter(
                ...
                c=[colors[i]], label=class_names[cls], alpha=0.7, s=50
            )
        ...
        axes[1].bar(range(1, len(cumsum) + 1), pca_full.explained_variance_ratio_, 
    ...
    def plot_class_trajectories(self, df: pd.DataFrame, save_path: Path | None = None) -> None:
        ...
        for idx, cls in enumerate(classes):
            ...
            ax.fill_between(
                time_steps, mean - std, mean + std,
                alpha=0.3, color="blue", label="±1 Std"
            )
            ax.fill_between(
                time_steps, mean - 2*std, mean + 2*std,
