...
class ClassifierFactory:
    ...
    def get_ensemble_classifiers(self) -> dict[str, ClassifierMixin]:
        ...
        return {
            ...
            "AdaBoost": AdaBoostClassifier(
                n_estimators=50, random_state=self.random_seed
            ),
        }
    ...
    def get_linear_classifiers(self) -> dict[str, ClassifierMixin]:
        ...
        return {
            "LogisticRegression_L2": LogisticRegression(
                ...
                random_state=self.random_seed, n_jobs=self.n_jobs
            ),
            ...
            "RidgeClassifier": RidgeClassifier(
                alpha=1.0, random_state=self.random_seed
            ),
            "SGDClassifier": SGDClassifier(
                loss="hinge", penalty="l2", max_iter=1000,
                random_state=self.random_seed, n_jobs=self.n_jobs
            ),
    ...
    def get_neural_network_classifiers(self) -> dict[str, ClassifierMixin]:
        ...
        return {
            ...
            "MLP_L2": MLPClassifier(
                hidden_layer_sizes=(100, 50), max_iter=500,
                alpha=0.01, random_state=self.random_seed, early_stopping=True
            ),
        }
    ...
    def get_boosting_classifiers(self) -> dict[str, ClassifierMixin]:
        ...
        return {
            ...
            "XGBoost": XGBClassifier(
                n_estimators=100, random_state=self.random_seed,
                use_label_encoder=False, eval_metric="mlogloss", n_jobs=self.n_jobs
            ),
            ...
            "CatBoost": CatBoostClassifier(
                iterations=100, random_state=self.random_seed,
                verbose=False, thread_count=self.n_jobs if self.n_jobs > 0 else -1
            ),
        }
    ...
    def get_other_classifiers(self) -> dict[str, ClassifierMixin]:
        ...
        return {
            ...
            "DummyStratified": DummyClassifier(strategy="stratified", random_state=self.random_seed),
        }
    ...
    def get_voting_ensemble(
        ...
        if base_classifiers is None:
            base_classifiers = {
                "rf": RandomForestClassifier(n_estimators=100, random_state=self.random_seed, n_jobs=self.n_jobs),
                "xgb": XGBClassifier(n_estimators=100, random_state=self.random_seed, n_jobs=self.n_jobs, eval_metric="mlogloss"),
    ...
    def get_stacking_ensemble(
        ...
        if base_classifiers is None:
            base_classifiers = {
                "rf": RandomForestClassifier(n_estimators=50, random_state=self.random_seed, n_jobs=self.n_jobs),
                "xgb": XGBClassifier(n_estimators=50, random_state=self.random_seed, n_jobs=self.n_jobs, eval_metric="mlogloss"),
                "svc": SVC(probability=True, random_state=self.random_seed),
            }
    ...
    def get_classifier_by_name(self, name: str, **kwargs: Any) -> ClassifierMixin:
        ...
        if name not in all_classifiers:
            raise ValueError(f"Unknown classifier: {name}. Available: {list(all_classifiers.keys())}")
    ...
    def get_quick_classifiers(self) -> dict[str, ClassifierMixin]:
        ...
        return {
            ...
            "XGBoost": XGBClassifier(
                n_estimators=50, random_state=self.random_seed,
                eval_metric="mlogloss", n_jobs=self.n_jobs
            ),
        }
