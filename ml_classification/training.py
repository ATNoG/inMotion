...
class TrainingPipeline:
    ...
    def __init__(self, config: Config) -> None:
        ...
        self.factory = ClassifierFactory(
            random_seed=config.random_seed, n_jobs=config.n_jobs
        )
    ...
    def train_single_classifier(
        ...
        try:
            ...
            return ClassifierResult(
                name=name, accuracy=0.0, balanced_accuracy=0.0,
                precision=0.0, recall=0.0, f1_score=0.0,
                cv_mean=0.0, cv_std=0.0, train_time=0.0
            )
        ...
        if hasattr(model, "feature_importances_"):
            ...
            feature_importances = np.abs(model.coef_).mean(axis=0) if model.coef_.ndim > 1 else np.abs(model.coef_)
            self.feature_importances[name] = feature_importances
    ...
    def train_all_classifiers(
        ...
        for i, (name, classifier) in enumerate(classifiers.items()):
            ...
            if verbose:
                print(f"Accuracy: {result.accuracy:.4f}, CV: {result.cv_mean:.4f}±{result.cv_std:.4f}, Time: {format_time(result.train_time)}")
