...
class OptunaOptimizer:
    ...
    def optimize_classifier(
        ...
        cv = StratifiedKFold(n_splits=self.config.n_cv_folds, shuffle=True, random_state=self.config.random_seed)
        ...
        def objective(trial: optuna.Trial) -> float:
            ...
            try:
                ...
                scores = cross_val_score(model, X, y, cv=cv, scoring="accuracy", n_jobs=self.config.n_jobs)
                return scores.mean()
    ...
    def get_optimized_classifier(
        ...
        best_params, _ = self.optimize_classifier(
            classifier_class, classifier_name, X, y, n_trials
        )
